...
router.get("/",checkAdmin,async function (req,res){
  ...
  var recentWorkout=await Workout.find({isDeleted:false}).sort({created:-1}).limit(5);
...
router.post(
  "/workoutadd",checkAdmin,upload.single("cover"),
...
router.get("/workoutlist",checkAdmin,async function(req,res){
  var workouts=await Workout.find({createdBy: req.session.admin.id,isDeleted:false,});
...
router.get("/workoutdetail/:id",checkAdmin,async function(req,res){
  var workout=await Workout.findOne({
    _id:req.params.id,createdBy: req.session.admin.id,
  });
  var challenges=await Challenge.find({workoutId:req.params.id});
  res.render("admin/workout/detail",{workout:workout,challenges:challenges});
});
...
res.render("admin/workout/update",{workout:workout});
});
router.post("/workoutupdate",checkAdmin,upload.single("cover"),async function(req,res){
  var update={
...
router.get("/challengeadd",checkAdmin,async function(req,res){
  const workouts=await Workout.find({
    createdBy : req.session.admin.id,isDeleted:false,
  });
...
router.post("/challengeadd",checkAdmin,uploadVideo.single("video"),
async function(req,res){
...
router.post("/challengeupdate",checkAdmin,uploadVideo.single("video"),async function(req,res){
  ...
  if(req.file){
    if(challenge.video){
      try{
        ...
        console.log("Video delete errors",e);
      }
    }
...
router.get("/challengedelete/:id",checkAdmin,async function(req,res){
  const data=await Challenge.findByIdAndDelete({
    _id:req.params.id,
    createdBy:req.session.admin.id,
  });
  if(data.video){
...
router.post("/foodadd",checkAdmin,upload.single("image"),async function(req,res){
  var food=new Food();
...
router.get("/foodlist",checkAdmin,async function(req,res){
  const foods=await Food.find().populate("createdBy","name");
...
router.post("/foodupdate",checkAdmin,upload.single("image"),async function(req,res){
