...
router.get('/',checkUser,async function(req, res, next) {
  const user=await User.findById(req.session.user.id).populate("activeWorkout","title")
...
router.post("/active",checkUser,async function(req,res){
  try{
    ...
    const user=await User.findByIdAndUpdate(req.session.user.id,{$set:update,
    });
...
router.post("/completeChallenge",checkUser,async function(req,res){
  ...
  console.log(update);
  try{
    const data=await User.findByIdAndUpdate(req.session.user.id,{$set: update});
    req.session.user.currentDay=update.currentDay;
    res.json({status:true});
  }catch(e)
  {
...
router.post("/reset",checkUser,async function(req,res){
  ...
  try {
    const data=await User.findByIdAndUpdate(req.session.user.id,{$unset:update});
